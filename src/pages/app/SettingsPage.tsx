    ...
    <div data-testid="settings-page">
      ...
      <Tabs.Root defaultValue="profile" className="grid gap-5 lg:grid-cols-[220px_1fr]">
        ...
        <div>
          ...
          <Tabs.Content value="plan"><SettingsCard title="Plan & billing" description="Review your current subscription and usage limits."><div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-brand-200 bg-brand-50 p-5"><div><div className="flex items-center gap-2"><h3 className="font-bold">Team plan</h3><Badge>Active</Badge></div><p className="mt-1 text-sm text-slate-500">$149 per user / month · 8 seats</p></div><Button variant="outline">Manage billing</Button></div><div className="mt-5 grid gap-3 sm:grid-cols-3">{[["AI queries", "1,481 / 2,500"], ["Seats", "8 / 10"], ["Private sources", "12 / 25"]].map(([label, value]) => <div key={label} className="rounded-lg border p-4"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 font-bold">{value}</p></div>)}</div></SettingsCard></Tabs.Content>
